...
| Markdown twins | `proxy.ts` + `app/md/[section]/[slug]/route.ts` | Every note/project at `<url>.md`, YAML frontmatter, built from the same source as the HTML — cannot drift |
...
| `rss.xml` | `app/rss.xml/route.ts` | Full-content feed of the current collection; historical GUIDs survive source moves |
...
- **"Copy page"** (the markdown-twin copy button) on every note and project page (`components/site/share-row.tsx` and `copy-button.tsx`) — readers paste our
