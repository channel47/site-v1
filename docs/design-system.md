...
| `Collection` | Balanced pairs at a maximum 980px width, sized by available width. Phones up to 520px use alternating objects, one per row. Each square object represents one piece, with a permanently visible gallery label: title, stationary arrow, and subordinate subject. Labels participate in normal flow and align with the artwork's inset. Artwork and caption share one wrapper, so they tilt, lift, and settle together. Hover/focus changes the title and arrow to blue and slowly lifts the whole piece on larger screens. Mobile opens with one tap. |
...
| `SharePopover` | A compact Share button beside the byline opens a right-aligned panel with Copy link, Copy page, X and LinkedIn. It uses the existing feedback timing and settle curve, with reduced motion respected. Escape, outside click, focus leaving the control, and successful selection close it. Escape and selection restore focus to the trigger; failed copies remain available to retry. Sharing appears only beside the byline. |
