...
| `Collection` | Balanced pairs at a maximum 980px width, sized by available width. Phones up to 520px use alternating objects, one per row. Each square object represents one piece, with a permanently visible gallery label: title, stationary arrow, and subordinate subject. Labels participate in normal flow, align with the artwork's inset, and remain upright. Hover/focus changes the title and arrow to blue and slowly lifts the artwork on larger screens. Mobile opens with one tap. |
