...
| Newsletter activity | A one-sentence introduction links to About, followed by a 12-week calendar of public commits and completed email sends. Neutral square intensity encodes commit counts; blue envelope markers identify sends. Hover previews a day; tapping selects it. A single full-calendar keyboard target supports arrow keys, Home and End, with a selected-day announcement. Keep the readout below the grid and RSS beneath the section. Los Angeles dates, source accounts and actual coverage dates remain visible. Selection fades use the existing 320ms timing; reduced motion removes the fade. Email subjects and bodies stay out of the calendar. |
