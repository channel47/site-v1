...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!entry) notFound()
  const artwork = getCollectionItems().find((item) => item.href === `/projects/${slug}`)?.src
  return renderOgImage({ kicker: "Project", title: entry.item.title, description: entry.item.description, accent: TYPE_ACCENTS.project, artwork })
}
